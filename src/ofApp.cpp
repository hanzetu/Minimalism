...
void ofApp::update(){
    ...
    waveform.clear(); // waveform is ofPolyline
    ...
    for(size_t i = 0; i < lastBuffer.getNumFrames(); i++) {
        ...
        waveform.addVertex(x, y + randomness * ofRandom(-3.3,3.3));
    }
...
void ofApp::audioOut(ofSoundBuffer &outBuffer) {
    ...
    float frequency = 172.5+randomness*(ofRandom(-5,5)*smooth[0].tick(1));
    ...
    for(size_t i = 0; i < outBuffer.getNumFrames(); i++) {
        ...
        float sampleMid = sin(wavePhase * 1.5 + randomness)*smooth[1].tick(1);
        float sampleHi = sin(wavePhase * 2.0 + randomness)*smooth[2].tick(1);
...
void ofApp::keyPressed(int key){
    if (key == OF_KEY_UP && randomness < 7) {
        randomness = randomness+ 0.1;
    }
    else if (key == OF_KEY_DOWN && randomness > 0) {
        randomness= randomness - 0.1;
    }
}
